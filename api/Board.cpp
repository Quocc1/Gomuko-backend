...
Board::Board(UInt8 boardSize_) {
	...
	blockedFiveInvalid = false;  // Initialize the flag to false by default
}
...
bool Board::check5InLine(Pos origin, Delta d, Piece p) {
	...
	// Check if we have exactly 5 in a row
	if (count1 + count2 == 4) {  // 4 + 1 (origin) = 5
		// If the rule is disabled, any five in a row is a win
		if (!blockedFiveInvalid) {
			return true;
		}
		
		// Check if both ends are blocked
		Pos leftEnd = origin + d;  // One step before the first stone
		Pos rightEnd = pos;        // One step after the last stone
		
		// Check if both ends are blocked by opponent's pieces or board boundaries
		bool leftBlocked = !isInBoard(leftEnd) || get(leftEnd) == Opponent(p);
		bool rightBlocked = !isInBoard(rightEnd) || get(rightEnd) == Opponent(p);
		
		// Only return true if at least one end is not blocked
