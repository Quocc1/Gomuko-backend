...
Pattern Evaluator::shortLinePattern(array<Piece, 9> & line) {
	...
	return getType(len, len2, count, block > 0, len2 - count, block);
}
// ͬ��˫������
...
bool Evaluator::checkFive(array<Piece, 9> & line, int i) {
	...
	Piece p = line[i];
	int count = 1;
	for (int j = i - 1; j >= 0 && line[j] == p; j--) count++;
	for (int j = i + 1; j < 9 && line[j] == p; j++) count++;

	if (count >= 5) {
		// Check if both ends are blocked when blockedFiveInvalid is true
		if (board->getBlockedFiveInvalid()) {
			// Find the actual leftmost and rightmost positions of the five
			int leftPos = i, rightPos = i;
			// Find leftmost position
			while (leftPos > 0 && line[leftPos - 1] == p) leftPos--;
			// Find rightmost position  
			while (rightPos < 8 && line[rightPos + 1] == p) rightPos++;
			
			// Check positions just outside the five
			int left = leftPos - 1;
			int right = rightPos + 1;
			bool leftBlocked = left < 0 || line[left] == Opponent(p);
			bool rightBlocked = right >= 9 || line[right] == Opponent(p);
...
// ��ö�Ӧ�ĵ�������
Pattern Evaluator::getType(int length, int fullLength, int count, bool block, int jump, int blockCount) {
	if (length < 5) return DEAD;
	if (count >= 5) {
		// Check for blocked five rule
		if (board->getBlockedFiveInvalid()) {
			// If both ends are blocked (blockCount == 2), this is not a valid winning five
			if (blockCount >= 2) {
				return DEAD;  // Blocked five is invalid
			}
		}
		return F5;
	}
